function [D,CollectionEfficiency]=laboratorycollectionefficiencyvssize
%Prototype constants
...
Dp= linspace(2*10^-6,80*10^-6,100); %potential diameters of water droplet (m)
...
Ri=Ri(1); %pitot inner tube radius (m)
R=R(1); %pitot outer tube radius (m)
...
D(3)=init(4); %ode of particle y-velocty
D(4)=((18*u)/(rhop*Dp^2))*(VFy-init(4))*z; %ode of particle y-acceleration
D=D';
end